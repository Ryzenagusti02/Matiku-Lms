...
export type QuestionType = 'multiple_choice' | 'multiple_response' | 'essay';
...
export interface Question {
  ...
  options: string[];
  correct_answer_index?: number | null; // For single multiple_choice
  correct_answers?: number[]; // For multiple_response
  essay_keywords?: string[]; // For essay
}
...
export interface ExamAttempt {
  ...
  student_uid: string;
  score: number;
  answers: { [questionId: string]: any }; // number (MC), number[] (MR), string (Essay)
  started_at: string;
