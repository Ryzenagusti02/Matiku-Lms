...
const DataSiswa: React.FC<DataSiswaProps> = ({ students, refreshData, addNotification }) => {
    ...
    const handleSaveStudent = async () => {
        ...
        if (!grade || !studentClass || !user) {
            Swal.fire({ title: 'Error', text: 'Tingkat dan Kelas harus diisi.', icon: 'error', background: document.documentElement.classList.contains('dark') ? '#1f2937' : '#ffffff', color: document.documentElement.classList.contains('dark') ? '#e5e7eb' : '#111827' });
            return;
        }

        if (!currentStudent.id && !studentEmail.trim()) {
            Swal.fire({ title: 'Error', text: 'Email siswa harus diisi saat menambah siswa baru.', icon: 'error', background: document.documentElement.classList.contains('dark') ? '#1f2937' : '#ffffff', color: document.documentElement.classList.contains('dark') ? '#e5e7eb' : '#111827' });
            return;
        }
        ...
        try {
            ...
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: `Siswa berhasil ${currentStudent.id ? 'diperbarui' : 'ditambahkan'}.`, showConfirmButton: false, timer: 3000, timerProgressBar: true, background: document.documentElement.classList.contains('dark') ? '#1f2937' : '#ffffff', color: document.documentElement.classList.contains('dark') ? '#e5e7eb' : '#111827' });
            refreshData();
            handleCloseModal();
        } catch (error: any) {
    ...
    const handleDelete = async (student: Student) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire({ title: 'Dihapus!', text: 'Data siswa telah dihapus.', icon: 'success', background: document.documentElement.classList.contains('dark') ? '#1f2937' : '#ffffff', color: document.documentElement.classList.contains('dark') ? '#e5e7eb' : '#111827' });
                addNotification(`Siswa '${student.name}' telah dihapus dari kelas Anda.`);
                refreshData();
            } catch (error: any) {
                console.error("Error deleting student:", error.message);
                Swal.fire({ title: 'Error!', text: error.message || 'Gagal menghapus data siswa.', icon: 'error', background: document.documentElement.classList.contains('dark') ? '#1f2937' : '#ffffff', color: document.documentElement.classList.contains('dark') ? '#e5e7eb' : '#111827' });
            }
        }
    };
    ...
    return (
        <div className="animate-fade-in-up">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-3xl font-medium text-gray-900 dark:text-white">Data Siswa</h3>
                <div className="flex gap-2">
                    <PdfExportButton targetRef={contentRef} fileName="Data_Siswa" />
                    ...
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            {students.map(student => (
                                <tr key={student.id} className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <td className="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">{student.name}</td>
                                    <td className="px-6 py-4">{student.grade}</td>
                                    <td className="px-6 py-4">{student.class}</td>
                                    <td className="px-6 py-4 text-right">
                                        <button onClick={() => handleOpenModal(student)} className="text-gray-500 dark:text-gray-400 hover:text-blue-400 mr-4">
                                            <EditIcon className="w-5 h-5" />
                                        </button>
                                        <button onClick={() => handleDelete(student)} className="text-gray-500 dark:text-gray-400 hover:text-red-400">
            ...
            {isModalOpen && (
                 <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-lg border border-gray-700">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">{currentStudent.id ? 'Edit Data Siswa' : 'Tambah Siswa ke Kelas'}</h2>
                        ...
                        <div className="space-y-4">
                            {!currentStudent.id ? (
                                <input type="email" placeholder="Email Siswa" value={studentEmail} onChange={(e) => setStudentEmail(e.target.value)} className="w-full p-2 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 border border-gray-300 dark:border-gray-600 rounded-md" />
                            ) : <p className="text-gray-300">Nama Siswa: <span className="font-semibold">{currentStudent.name}</span></p>}
                             <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Tingkat</label>
                                    <select value={currentStudent.grade || ''} onChange={(e) => setCurrentStudent({ ...currentStudent, grade: e.target.value })} className="w-full p-2 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 border border-gray-300 dark:border-gray-600 rounded-md">
                                        {gradeLevels.map(level => <option key={level} value={level}>{level}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Kelas</label>
                                    <input type="text" placeholder="Cth: A" value={currentStudent.class || ''} onChange={handleClassChange} className="w-full p-2 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 border border-gray-300 dark:border-gray-600 rounded-md" maxLength={3} />
                                </div>
                            </div>
                        </div>
