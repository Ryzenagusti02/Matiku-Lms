...
import { HomeIcon, BookOpenIcon, UsersIcon, ClipboardCheckIcon, BarChartIcon, SparklesIcon, FileTextIcon, FolderIcon, ClipboardListIcon, GridIcon } from './icons';
...
const NavLink: React.FC<{
    ...
    return (
        <a
            ...
            className={`flex items-center px-4 py-2 mt-5 transition-colors duration-200 transform rounded-md ${isActive ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-200'}`}
        >
            {icon}
...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, setCurrentPage, isSidebarOpen, setIsSidebarOpen }) => {
    ...
    return (
        <>
            ...
            <div className={`fixed inset-y-0 left-0 z-30 w-64 px-4 py-5 overflow-y-auto bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 shadow-sm transform lg:translate-x-0 lg:static lg:inset-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-200 ease-in-out`}>
                ...
                <nav className="mt-2 flex-grow">
                    ...
                    <NavLink pageName="papan-matematika" currentPage={currentPage} onClick={handleNavClick} icon={<span className="font-serif italic font-bold text-lg w-6 h-6 flex items-center justify-center -ml-1">f(x)</span>} label="Papan Matematika" />
