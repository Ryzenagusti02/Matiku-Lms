...
const StudentActivity: React.FC<StudentActivityProps> = ({ students }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 dark:shadow-lg h-full">
            <div className="flex items-center mb-4">
                <UsersIcon className="w-6 h-6 text-blue-600 dark:text-blue-400"/>
                <h4 className="ml-2 text-xl text-gray-900 dark:text-gray-200 font-semibold">Aktivitas Siswa Terbaru</h4>
            </div>
            <div className="space-y-4">
                {recentStudents.length > 0 ? (
                    recentStudents.map(student => (
                        <div key={student.id} className="flex items-center justify-between text-sm">
                            <p className="text-gray-700 dark:text-gray-300 font-medium">{student.name}</p>
