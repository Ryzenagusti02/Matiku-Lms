...
const ModulAjar: React.FC<ModulAjarProps> = ({ modules, refreshData, addNotification }) => {
    ...
    const handleGenerateDescription = async () => {
        ...
        try {
            ...
            setCurrentModule({ ...currentModule, description: result });
        } catch (error: any) {
            Swal.fire({
                title: 'Error AI',
    ...
    const handleSaveModule = async () => {
        ...
        try {
            ...
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Modul berhasil disimpan!', showConfirmButton: false, timer: 3000, timerProgressBar: true, background: document.documentElement.classList.contains('dark') ? '#1f2937' : '#ffffff', color: document.documentElement.classList.contains('dark') ? '#e5e7eb' : '#111827' });
            refreshData();
            handleCloseModal();
        } catch (error: any) {
    ...
    const handleDelete = async (module: Module) => {
        ...
        if (result.isConfirmed) {
            ...
            try {
                ...
                Swal.fire({ title: 'Dihapus!', text: 'Modul telah dihapus.', icon: 'success', background: document.documentElement.classList.contains('dark') ? '#1f2937' : '#ffffff', color: document.documentElement.classList.contains('dark') ? '#e5e7eb' : '#111827' });
                addNotification(`Modul '${module.title}' telah dihapus.`);
                refreshData();
            } catch (error: any) {
                console.error("Error deleting module:", error.message);
                Swal.fire({ title: 'Error!', text: error.message || 'Gagal menghapus modul.', icon: 'error', background: document.documentElement.classList.contains('dark') ? '#1f2937' : '#ffffff', color: document.documentElement.classList.contains('dark') ? '#e5e7eb' : '#111827' });
            }
        }
    };
    ...
    return (
        <div className="animate-fade-in-up">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-3xl font-medium text-gray-900 dark:text-white">Modul Ajar</h3>
                <div className="flex gap-2">
                    <PdfExportButton targetRef={contentRef} fileName="Modul_Ajar" />
            ...
            <div ref={contentRef} className="bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700 rounded-lg rounded-lg overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            {modules.map(module => (
                                <tr key={module.id} className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                    <td className="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">{module.title}</td>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <button onClick={() => handleOpenModal(module)} className="text-gray-500 dark:text-gray-400 hover:text-blue-400 mr-4">
                                            <EditIcon className="w-5 h-5" />
                                        </button>
                                        <button onClick={() => handleDelete(module)} className="text-gray-500 dark:text-gray-400 hover:text-red-400">
                ...
                <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-lg border border-gray-700 max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">{currentModule.id ? 'Edit Modul Ajar' : 'Tambah Modul Baru'}</h2>
                        ...
                        <div className="space-y-4">
                            <input type="text" placeholder="Judul Modul" value={currentModule.title || ''} onChange={(e) => setCurrentModule({ ...currentModule, title: e.target.value })} className="w-full p-2 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 border border-gray-300 dark:border-gray-600 rounded-md" />
                            <div className="relative">
                                <textarea placeholder="Deskripsi Modul..." value={currentModule.description || ''} onChange={(e) => setCurrentModule({ ...currentModule, description: e.target.value })} className="w-full p-2 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 border border-gray-300 dark:border-gray-600 rounded-md h-32 pr-10" />
                                <button
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Tenggat Waktu (Opsional)</label>
                                <input type="date" value={currentModule.due_date ? currentModule.due_date.split('T')[0] : ''} onChange={(e) => setCurrentModule({ ...currentModule, due_date: e.target.value })} className="w-full p-2 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-200 border border-gray-300 dark:border-gray-600 rounded-md" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Upload File (PDF)</label>
                                <FileUpload onFileSelect={handleFileSelect} acceptedTypes="application/pdf" maxSizeMB={5} />
                                {moduleFile && <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">File dipilih: {moduleFile.name}</p>}
                                {currentModule.file_name && !moduleFile && <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">File saat ini: {currentModule.file_name}</p>}
                            </div>
                        </div>
