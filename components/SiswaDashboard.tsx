...
const SiswaDashboard: React.FC<SiswaDashboardProps> = ({ user, studentData, exams, attempts, assignments, submissions, refreshData }) => {
    ...
    const handleSubmitAssignment = async () => {
        ...
        try {
            ...
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Tugas berhasil dikumpulkan!', showConfirmButton: false, timer: 3000, timerProgressBar: true, background: document.documentElement.classList.contains('dark') ? '#1f2937' : '#ffffff', color: document.documentElement.classList.contains('dark') ? '#e5e7eb' : '#111827' });
            refreshData();
            closeSubmissionModal();

        } catch (error: any) {
            Swal.fire({ title: 'Error', text: error.message || 'Gagal mengumpulkan tugas.', icon: 'error', background: document.documentElement.classList.contains('dark') ? '#1f2937' : '#ffffff', color: document.documentElement.classList.contains('dark') ? '#e5e7eb' : '#111827' });
        } finally {
